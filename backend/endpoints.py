...
def to_out(movie: Movie, favs: set) -> MovieOut:
    return MovieOut(
        ...
        is_favorite=movie.tconst in favs,
    )
...
def save_survey(payload: SurveyPayload, db: Session = Depends(get_db)):
    ...
    pref = Preference(
        ...
        actors=",".join(payload.actors),
        directors=",".join(payload.directors),
    )
    db.add(pref)
    db.commit()
    return {"status": "ok"}
...
def lucky(session_key: str = Query(...), db: Session = Depends(get_db)):
    user = get_user(db, session_key)
    favs = get_favs_set(db, user)
    q = db.query(Movie).filter(Movie.rating >= 7.0, Movie.votes >= 5000)
...
def mood(session_key: str = Query(...), mood: str = Query(...), db: Session = Depends(get_db)):
    user = get_user(db, session_key)
    favs = get_favs_set(db, user)
    genres = MOOD_GENRE_MAP.get(mood.lower(), [])
    q = db.query(Movie).filter(Movie.rating >= 6.5)
    if genres:
        q = q.filter(or_(*[Movie.genres.ilike(f"%{g}%") for g in genres]))
    if user and user.preferences and user.preferences.content_type != "both":
        q = q.filter(Movie.type == user.preferences.content_type)
...
def toggle_favorite(payload: FavoritePayload, db: Session = Depends(get_db)):
    ...
    existing = db.query(Favorite).filter(Favorite.user_id == user.id, Favorite.tconst == payload.tconst).first()
    if existing:
        db.delete(existing)
        db.commit()
